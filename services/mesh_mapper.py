...
def search_mesh_term(condition, delay=0.35, filter_diseases_only=True, entrez_email:str = None, tool_name: str = None):
    """
    ...
    """
    email = os.getenv("ENTREZ_EMAIL") if entrez_email is None else entrez_email
    tool = os.getenv("TOOL_NAME") if tool_name is None else tool_name
    ...
    if email and tool:
        try:
            ...
            # Step 1: Search for MeSH term using esearch, sorting by relevance
        ...
        logger.Warning("Warning - please set a TOOL_NAME and ENTREZ_EMAIL in .env (see .env.example file)")